...
# TargetFramework
For Demo13, the TargetFramework for the server.exe program is netcoreapp3.0.
...
#Demo13 Server as a Windows Service and Linux Daemon
...
Create a new Solution Folder and rename it to PublishedService. Under the Demo13 subfolder create a new Windows folder called PublishedService. 

### Create a new Server Publish Profile
Right-Click `Server.csproj file` in the Solution explorer, click `Publish...`, then Start, then select Folder as the Publish target. Chose a folder, use the folder picker, select the PublishedService folder under Demo13.
